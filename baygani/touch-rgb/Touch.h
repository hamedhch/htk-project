...
volatile int R=0 , G=160, B=160 , rgb=0 , m2=0 ;
...
void Key_Touch()
{
	...
	if(Count_S1 < Count_C1 - 50)
	{
		...
		if(Count_T1> 5000 && q==0){Count_T1 = 5000;}
	}
	else if(Count_S1 > Count_C1 -10 )
	{
		if(Count_T1 >= FINGER_FOCUS_ON  && Count_T1 <= FINGER_FOCUS_ON+500  && rgb==0)
 		{
 			if(q==1)q=0;
			else q=1;
			CalibrCount=0;	
