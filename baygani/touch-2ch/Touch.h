...
volatile unsigned char i=0,j=0, counter=0,CalibrCount1=0,CalibrSample1=0;
...
volatile int ncount1=0;
volatile int counter11=0;
volatile int counter12=0;
volatile int counter13=0;
volatile bit q1=0;

volatile int ncount2=0;
volatile int counter21=0;
volatile int counter22=0;
volatile int counter23=0;
volatile bit q2=0;
...
void CalibrTuoch()
{
	...
	{	
		...
		for(i=0; i <20 ; i++)
		{
			...
			if(j == 0){Count_S1 = (unsigned int )((_tkm016dh <<8) | _tkm016dl);Count_C1 += Count_S1;}
			if(j == 1){Count_S2 = (unsigned int )((_tkm016dh <<8) | _tkm016dl);Count_C2 += Count_S2;}
...
void Key_Select(char Sel)
{
	switch(Sel)
	{
		case 0: ROcTouch();KOcTouch();TouchStart(1);KeyTouch1();
		while(!_tkrcov ){} ;Count_S1 = (unsigned int)((_tkm016dh <<8) | _tkm016dl);
		CalibrCount1++;
		if(CalibrCount1>100)
		{
			CalibrCount1=0;	
		...
		case 1: ROcTouch();KOcTouch();TouchStart(1);KeyTouch2();
		while(!_tkrcov ){}; Count_S2 = (unsigned int)((_tkm016dh <<8) | _tkm016dl);
		CalibrCount2++;
		if(CalibrCount2>100)
		{
			CalibrCount2=0;	
