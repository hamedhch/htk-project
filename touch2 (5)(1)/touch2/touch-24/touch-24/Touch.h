...
volatile bit  Start=0,Lock=0,LernRFblink=0 , LernRF=0,Frist=0,Finish=0;
	...
	if(data_rf==0){
	 ...
	 else
	 {
		...
		if(Frist==1 && Finish==0)
        {
			...
			{
				Bit--;
				if(Timedown>22 && Timedown< 45){
					Buffer = Buffer << 1;  
				}
				else if(Timedown>7 && Timedown< 18){
					Buffer = Buffer << 1;
					Buffer = Buffer + 1;
				}
                else{
