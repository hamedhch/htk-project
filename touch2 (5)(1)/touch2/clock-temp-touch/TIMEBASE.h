...
void __attribute((interrupt(0x28))) TB1_ISR(void)
{
	...
	if(Litgh){
		...
		switch(State)
		{
			...
			case  14: DispTemp();State=0;Cpin=1; break;
			...
			case 30: DispTimer(Belinker,1);State=0;Belinking=0; break;
	...
	*/
	...
	if(Bip==1)
	{
		Biper++;
		if(Biper<200){BUZ=1;}
		if(Biper>200 && Biper<300){BUZ=0;}
		if(Biper>300 && Biper<500){BUZ=1;}
		if(Biper>500){BUZ=0;}
		if(Biper>2000){Biper=0;}
	}
	else{Biper=0;BUZ=0;}
