...
volatile int R=160 , G=160, B=0 , rgbchange=0 , rgbauto=0 , rgb1=0  , rgb2=0 , rgbpals=0 , counterrgb=15000;
...
void Key_Touch()
{
	...
	if(Count_S1 < Count_C1 - 50)
	{
		...
		if(Count_T1>300 && q==1 && rgbchange==0){
	...
	else if(Count_S1 > Count_C1 -10 )
	{
		...
		if(Count_T1 >= FINGER_FOCUS_ON  && Count_T1 <= FINGER_FOCUS_ON+300 && rgbchange==0)
	 		{
	 			if(q==1)q=0;
				else q=1;
...
void __attribute((interrupt(0x04))) int0(void)
{
	...
	if((rgbchange || rgbauto) && q==1 && counterrgb>=50){
