...
volatile unsigned char Frist,CountEdge, Zero_Time,Zero_Flag,Trig,Triger,Shift,Set,Count,CountSW,Backup,Detect;
...
{
	CountSW++;
	if(CountSW>=0 & CountSW<=25)
 	{
 		STM_ENABLE();
 	}
 	if(CountSW>=26 & CountSW<45)
 	{
 		STM_DISABLE();
 	}
 	if(CountSW>=46)
